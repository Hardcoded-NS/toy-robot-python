...
class Robot(object):
    ...
    def left(self):
        return self.place(self.location, self.facing - 0.5, self.table)
    ...
    def report(self):
        if self.table is not None:
            print(round(self.location.x), round(self.location.y)
                 , self.dirs[round(self.facing)])

    def place(self, location, facing, table):
        if table is not None and table.contains(location):
            return Robot(location, facing, table)
        else:
            return self
